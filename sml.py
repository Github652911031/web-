...
def save_model_picture_for_multipro(picture_save_path):
    ...
    pict_dct_staggered = pict_dct.reshape(pict_dct.shape[0], 3, -1)[:, :, :10].reshape(pict_dct.shape[0],
                                                                                       -1)  # to make the YBR staggered
    #     X= np.array(np.transpose(output,(1,2,0,3,4)),dtype=np.float16)## 必须强制转成float16 否则内存难以接受.
    model_pic = mixture.GaussianMixture(n_components=8).fit(pict_dct_staggered)  # k = 8, covariances='full'
